...
function updateTimer(i) {
  try {
    ...
    if (!negatives[i]) {
      ...
      // DETENER automáticamente cuando llega a -10:00 pero MANTENER la tarjeta activa
      if (remaining <= CONFIG.LIMITE_NEGATIVO) {
        // Detener el intervalo del timer
        clearInterval(timers[i]);
        timers[i] = null;
        
        // IMPORTANTE: NO llamar stopTimer() para mantener la UI activa
        // Fijar el display en exactamente -10:00
        updateTimerDisplay(i, CONFIG.LIMITE_NEGATIVO);
        
        // Guardar el estado final
        saveTimerState(i, CONFIG.LIMITE_NEGATIVO, false);
        
        // Mantener la tarjeta como activa visualmente pero sin timer corriendo
        // NO llamamos updateCardUI(i, false) para mantener los botones
        
        // Sonido y notificación
        audioManager.playSoundLoop(audioManager.alertEndSound);
        showToast(`Timer ${getNombreCasa(i)} alcanzó -10:00`, 'warning');
...
// También necesitas modificar la función stopTimer para manejar el caso cuando el timer ya está en -10:00
function stopTimer(i) {
  try {
    ...
    // Actualizar UI - ESTO removerá los botones y volverá al estado inicial
...
// Y actualizar actualizarContadorActivos para no contar los timers pausados en -10:00
function actualizarContadorActivos() {
  try {
    const contador = document.getElementById("contador-activos");
    if (!contador) return;
    
    // Contar solo timers que están realmente corriendo (no los pausados en -10:00)
